...
>(
  ...
  const [cachedData, setCachedData] = useState<FunctionReturnType<Query> | undefined>(() => {
    return queryClient.getQueryData(fullQueryKey) as FunctionReturnType<Query> | undefined;
  });
  ...
  useEffect(() => {
    const unsubscribe = queryClient.getQueryCache().subscribe((event) => {
      if (
        ...
        const data = queryClient.getQueryData(fullQueryKey) as FunctionReturnType<Query> | undefined;
        if (data !== undefined) {
          setCachedData(data);
        }
      }
