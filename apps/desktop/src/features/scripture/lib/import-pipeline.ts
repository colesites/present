...
import { db, type BibleVerse, type BibleVersion, type BibleBookRecord } from "./db";
...
async function importFromJson(
  ...
  const data = JSON.parse(content) as {
    version?: { id?: string; name?: string; code?: string };
    verses: (Partial<BibleVerse> & {
      bookId: string;
      chapter: number;
      verse: number;
      text: string;
  ...
  const version: BibleVersion = {
    id: versionId,
    name: bibleName,
    code: data.version?.code || bibleName.substring(0, 3).toUpperCase(),
  ...
  const books: BibleBookRecord[] = data.books || [];
    ...
    for (const verseElement of verseElements) {
      ...
      if (parts.length >= 3) {
        ...
        if (!text.trim() && verseElement.getAttribute("sID")) {
          ...
          let node: ChildNode | null = verseElement.nextSibling;
          let collectedText = "";
